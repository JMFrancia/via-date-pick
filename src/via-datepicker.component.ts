...
export class DatePickerOptions {
  ...
  constructor(obj?: IDatePickerOptions) {
    ...
    this.todayText = obj && obj.todayText ? obj.todayText : 'Today\'s date';
    this.clearText = obj && obj.clearText ? obj.clearText : 'Clear';
    this.width = obj && obj.width ? obj.width : 193;
...
@Component({
  ...
  templateUrl: './via-datepicker.component.html',
  styleUrls: ['./via-datepicker.component.css'],
...
export class DatePickerComponent implements ControlValueAccessor, OnInit {
  ...
  private enterDate(){
    let input:moment.Moment = moment(this.textinput.nativeElement.value, this.options.format, true);
    if (!input.isValid()) {
      this.setError('Invalid date');
    } else if (input.isAfter(this.options.maxDate)){
      this.setError('Requires earlier date');
    } else if (input.isBefore(this.options.minDate)){
      this.setError('Requires later date');
    } else {
      this.selectDate(null, input);
      this.clearError();
    }
  }
 ...
 private generateDaysInRange(date: moment.Moment, start: number, end: number, monthAdjustment: number): CalendarDate[] {
   ...
   for (let i = start; i <= end; i += 1) {
     const currentDate: moment.Moment = Moment(`${i}.${month + monthAdjustment}.${year}`, 'DD.MM.YYYY');
 ...
 /* Generates the calendar based on current options */
 generateCalendar() {
   ...
   let currentMonthDays: CalendarDate[] = this.generateDaysInRange(date, n, date.endOf('month').date(), 1);
   let nextMonthDays: CalendarDate[] = this.generateDaysInRange(date, 1, ((7 - (currentMonthDays.length % 7)) % 7), 2);
   this.days = currentMonthDays.concat(nextMonthDays);
 }

 /* Select a date - normally via mouseclick, but can be done programatically by passing 'null' as the first arg */
 selectDate(e: MouseEvent = null, date: moment.Moment) {
